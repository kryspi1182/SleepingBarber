...
int wait_limit = 10;
int debug = 0;
pthread_cond_t client_waiting = PTHREAD_COND_INITIALIZER;
pthread_cond_t barber_sleeping = PTHREAD_COND_INITIALIZER;
int chair_taken = 0;
int finished = 0;
int next_client = 0;
struct timespec ts;
...
void *Client(void *num)
{
    ...
    pthread_cond_signal(&client_waiting); // inform the barber that a client is waiting
    if(debug==1)
        add_waiting(nr);
    ...
    if(chair_taken)
        pthread_cond_wait(&barber_sleeping, &chair); // if the barber is busy with another client, wait
...
void *Barber()
{
    pthread_mutex_lock(&barber_mutex); // a mutex has to be locked to run a timedwait
    while(1)
    {
        if(finished == 1)
            return;

        pthread_cond_signal(&barber_sleeping); // send info about the barber napping
        if(pthread_cond_timedwait(&client_waiting, &barber_mutex, &ts)==-1) // wait for a while for a client, if none appears, the loop goes into another iteration
            continue;
    }
}
